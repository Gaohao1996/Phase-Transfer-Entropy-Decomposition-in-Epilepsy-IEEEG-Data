...
Epilepsy_data  = load('sz8_pre_clean.mat').pre_eeg; 
% Epilepsy_data  = load('sz8_ict_clean.mat').ict_eeg; 
...
for i = 1:num_windows
    connection_12_11(i) = PLV_adjacent_weighted(11,12, i);
end
...
xlabel('Channel');
ylabel('Channel');
title(['PLV Heatmap in Beta band at Time Window ', num2str(t_index)]);

%%Graph visualization at a certain time window
G = graph(PLV_adjacent_weighted(:,:,t_index), 'upper');  %
...
for t_index = 1: num_windows
    G1 = graph(PLV_adjacent_weighted(:,:,t_index), 'upper'); 
...
title('Edge Count of Beta Band functional network');
grid on;

%% Graph visualization
SOZ_nodes =[75,76];
special_nodes =[1,2,3,4,9,10,11,12,17,18,19,26]; %potential EZ channels

%% Save pictures of all time winodws
% SavePath = 'D:\Simulation_PhD\epilepsy_simulation\results\functional network\FC\FC_phase\Beta_FC\ict4';
% save_PLV_figure(num_windows, plv, SavePath)
% save_FN_figures(num_windows,PLV_adjacent_weighted, SOZ_nodes, special_nodes, SavePath)
